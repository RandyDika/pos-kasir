...
class view
{
    ...
    public function keranjang($id_member){
    $sql = "SELECT keranjang.*, barang.nama_barang, member.nm_member
            FROM keranjang
            JOIN barang ON keranjang.id_barang = barang.id_barang
            JOIN member ON keranjang.id_member = member.id_member
            WHERE keranjang.id_member = ?";
    $row = $this->db->prepare($sql);
    $row->execute([$id_member]);
    return $row->fetchAll();
}
public function penjualan_by_nota($id_nota) {
    ...
    {
        $sql ="SELECT SUM(total) as bayar FROM keranjang";
        $row = $this -> db -> prepare($sql);
        $row -> execute();
        $hasil = $row -> fetch();
        return $hasil;
